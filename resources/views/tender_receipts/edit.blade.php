...
@section('header')
    <div class="page-header">
        <h1><i class="glyphicon glyphicon-edit"></i> TenderReceipts / Edit #{{$tender_receipt->id}}</h1>
    </div>
@endsection
...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <form action="{{ route('tender_receipts.update', $tender_receipt->id) }}" method="POST">
                ...
                <div class="form-group @if($errors->has('company_name')) has-error @endif">
                    ...
                    <input type="text" id="company_name-field" name="company_name" class="form-control" value="{{ is_null(old("company_name")) ? $tender_receipt->company_name : old("company_name") }}"/>
                    ...
                    <input type="text" id="receipt_number-field" name="receipt_number" class="form-control" value="{{ is_null(old("receipt_number")) ? $tender_receipt->receipt_number : old("receipt_number") }}"/>
             ...
             {{--        <div class="form-group @if($errors->has('tender_id')) has-error @endif">
                    ...
                    <input type="text" id="tender_id-field" name="tender_id" class="form-control" value="{{ is_null(old("tender_id")) ? $tender_receipt->tender_id : old("tender_id") }}"/>
                     ...
                     <div class="form-group @if($errors->has('tender_id')) has-error @endif">
                       ...
                       <option value="" disabled selected>Select your option</option>
                        @foreach($tenders as $tender)
                          <option value="{{$tender->id}}" <?php if($tender->id == $tender_receipt->tender_id) echo 'selected'?>>{{$tender->tender_no}}</option>
                        @endforeach
                       </Select>
                   {{--  <input type="text" id="tender_id-field" name="tender_id" class="form-control" value="{{ old("tender_id") }}"/> --}}
                    ...
                    <input type="text" id="receiver_name-field" name="receiver_name" class="form-control" value="{{ is_null(old("receiver_name")) ? $tender_receipt->receiver_name : old("receiver_name") }}"/>
                    ...
                    <input type="text" id="password-field" name="password" class="form-control" value="{{ is_null(old("password")) ? $tender_receipt->password : old("password") }}"/>
